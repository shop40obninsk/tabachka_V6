...
def command_worker(message,chat_id):
    ...
    if str(message).replace("/", '').replace(f"{ispar_key} ", "") in get_dir(ispar) and ispar_key in str(message):
        ...
        for i in get_dir(ispar + str(message).replace("/", '').replace(f"{ispar_key} ", "") + "/"):
            path = ispar + str(message).replace("/", '').replace(f"{ispar_key} ", "") + "/" + str(i) + "/"
            print(path)
            i = get_description(path)
            mass = []
            for t in i[3]:
                mass.append([f"{t}", f"I:{i[1]}:{manufacturer}:{t}"])
            keyboard = Inline_Keyboard_Generator(mass)
            bot.send_photo(chat_id, photo=open(i[5], 'rb'), caption=i[0], reply_markup=keyboard)
...
def welcome(message):
    bot.send_message(message.chat.id, f"Добро пожаловать, {message.from_user.first_name} ! 🤍"
                     ...
                     reply_markup=Keyboard_Generator(["/Электронные сигареты","/Жижи","/Испарители","/Поделиться"]))
...
def get_message(message):
    ...
    if text=="Назад к производителям Испарителей":
        mass = []
        for i in get_dir(ispar):
            mass.append("/" + f"{ispar_key} "+str(i))
        mass.append("На главную")
